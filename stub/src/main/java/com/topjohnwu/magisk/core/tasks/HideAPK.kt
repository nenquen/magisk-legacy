...
object HideAPK {
    ...
    private suspend fun patchAndHide(activity: Activity, label: String): Boolean {
        ...
        if (!Shell.su("adb_pm_install $repack").exec().isSuccess) {
             ...
             Utils.toast("Kurulum başarısız. Lütfen root erişimini kontrol edin.", Toast.LENGTH_LONG)
        }
        return true
    }
    ...
    fun restore(activity: Activity) {
        ...
        Shell.su("adb_pm_install $apk").submit {
            if (!it.isSuccess) {
                 // NO FALLBACK TO APKInstall.install
                 Utils.toast("Geri yükleme başarısız. Lütfen root erişimini kontrol edin.", Toast.LENGTH_LONG)
            }
        }
    }
}
