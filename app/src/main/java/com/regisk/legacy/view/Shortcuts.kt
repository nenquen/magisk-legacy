...
object Shortcuts {
    ...
    fun addHomeIcon(context: Context) {
        ...
        val info = ShortcutInfoCompat.Builder(context, Const.Nav.HOME)
            .setShortLabel(context.getString(R.string.regisk))
            .setIntent(intent)
    ...
    private fun getShortCuts(context: Context): List<ShortcutInfo> {
        ...
        if (Info.env.regiskHide) {
            shortCuts.add(
                ShortcutInfo.Builder(context, Const.Nav.HIDE)
                    .setShortLabel(context.getString(R.string.regiskhide))
                    .setIntent(
                        Intent(intent).putExtra(Const.Key.OPEN_SECTION, Const.Nav.HIDE)
                    )
                    .setIcon(context.getIcon(R.drawable.sc_regiskhide))
                    .setRank(1)
                    .build()
            )
        }
