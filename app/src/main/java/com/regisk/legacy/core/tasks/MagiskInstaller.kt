...
abstract class RegiskInstallImpl protected constructor(
    ...
    private fun handleFile(uri: Uri): Boolean {
        ...
        try {
            uri.inputStream().buffered().use { src ->
                ...
                val filename = StringBuilder("regisk_patched-${BuildConfig.VERSION_CODE}_").run {
                    for (i in 1..5) {
...
abstract class RegiskInstaller(
    console: MutableList<String>,
    logs: MutableList<String>
) : RegiskInstallImpl(console, logs) {
    ...
    class FixEnv(private val callback: () -> Unit) : RegiskInstallImpl() {
        override suspend fun operations() = fixEnv()
