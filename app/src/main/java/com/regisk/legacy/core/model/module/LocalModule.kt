package com.regisk.legacy.core.model.module
...
class LocalModule(path: String) : Module() {
    ...
    companion object {
        ...
        private val PERSIST get() = "${Const.REGISKTMP}/mirror/persist/regisk"

        suspend fun installed() = withContext(Dispatchers.IO) {
            SuFile(Const.REGISK_PATH)
                .listFiles { _, name -> name != "lost+found" && name != ".core" }
                .orEmpty()
                .filter { !it.isFile }
                .map { LocalModule("${Const.REGISK_PATH}/${it.name}") }
