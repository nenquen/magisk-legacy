package com.regisk.legacy.core.tasks
...
import com.regisk.legacy.core.Provider
import com.regisk.legacy.core.utils.AXML
import com.regisk.legacy.core.utils.Keygen
import com.regisk.legacy.di.ServiceLocator
import com.regisk.legacy.ktx.writeTo
import com.regisk.legacy.utils.APKInstall
...
object HideAPK {
    ...
    private const val APP_NAME = "Regisk"
    ...
    private suspend fun patchAndHide(activity: Activity, label: String): Boolean {
        ...
        // Network removed â€” stub download skipped
        ...
        if (!Shell.su("adb_pm_install $repack").exec().isSuccess) {
             // NO FALLBACK TO APKInstall.install
             Utils.toast("Kurulum baÅŸarÄ±sÄ±z. LÃ¼tfen root eriÅŸimini kontrol edin.", Toast.LENGTH_LONG)
        }
        return true
    }
    ...
    fun restore(activity: Activity) {
        ...
        Shell.su("adb_pm_install $apk").submit {
            if (!it.isSuccess) {
                 // NO FALLBACK TO APKInstall.install
                 Utils.toast("Geri yÃ¼kleme baÅŸarÄ±sÄ±z. LÃ¼tfen root eriÅŸimini kontrol edin.", Toast.LENGTH_LONG)
            }
        }
    }
}
