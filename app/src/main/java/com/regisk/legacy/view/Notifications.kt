...
import com.regisk.legacy.core.Const.ID.PROGRESS_NOTIFICATION_CHANNEL
import com.regisk.legacy.core.Const.ID.UPDATE_NOTIFICATION_CHANNEL
...
import com.regisk.legacy.ktx.getBitmap
...
object Notifications {
    ...
    private fun updateBuilder(context: Context): Notification.Builder {
        return Notification.Builder(context).apply {
            val bitmap = context.getBitmap(R.drawable.ic_regisk_outline)
            setLargeIcon(bitmap)
            if (SDK_INT >= 26) {
                setSmallIcon(bitmap.toIcon())
                setChannelId(UPDATE_NOTIFICATION_CHANNEL)
            } else {
                setSmallIcon(R.drawable.ic_regisk_outline)
            }
        }
    }
    ...
    fun managerUpdate(context: Context) {
        ...
        val builder = updateBuilder(context)
            .setContentTitle(context.getString(R.string.regisk_update_title))
