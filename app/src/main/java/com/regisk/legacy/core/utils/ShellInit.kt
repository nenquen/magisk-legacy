package com.regisk.legacy.core.utils
...
import com.regisk.legacy.ktx.rawResource
    ...
    override fun init(context: Context, shell: Shell): Boolean {
        shell.newJob().apply {
            ...
            if (shell.isRoot) {
                ...
                add("export REGISKTMP=\$MAGISKTMP")
                add("export REGISKBIN=/data/adb/magisk")
...
class AppShellInit : BaseShellInit() {
    ...
    override fun init(context: Context, shell: Shell): Boolean {
        ...
        Const.REGISKTMP = getVar("REGISKTMP")
